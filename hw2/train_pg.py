...
def build_mlp(
    ...
    with tf.variable_scope(scope):
        ...
        for i in range(n_layers):
            x = tf.layers.dense(x, units=size, activation=activation, name='fc' + str(i))
        output = tf.layers.dense(x, units=output_size, activation=output_activation, name='fc' + str(n_layers))
    return output
...
def train_PG(exp_name='',
    ...
    if discrete:
        ...
        #  one_hot_ac = tf.one_hot(indices=sy_ac_na, depth=ac_dim)
        #  sy_logprob_n = tf.log(tf.reduce_sum(sy_logits_na * one_hot_ac, axis=1) + 1e-8

        sy_logits_na = build_mlp(sy_ob_no, ac_dim, "discrete_mlp", n_layers=n_layers, size=size, activation=tf.nn.relu,
                                 output_activation=None)
        sy_sampled_ac = tf.identity(tf.multinomial(sy_logits_na, num_samples=1)[:, 0],
                                    name="sampled_ac")  # Hint: Use the tf.multinomial op
        sy_logprob_n = -tf.nn.sparse_softmax_cross_entropy_with_logits(labels=sy_ac_na, logits=sy_logits_na)

    else:
        sy_mean = build_mlp(sy_ob_no, ac_dim, "cont_mlp", n_layers=n_layers, size=size, activation=tf.nn.relu,
                            output_activation=None)
        sy_logstd = tf.get_variable("logstd", shape=[ac_dim],
                                    dtype=tf.float32)  # logstd should just be a trainable variable, not a network output.
        sy_std = tf.exp(sy_logstd)

        z = tf.random_normal((tf.shape(sy_mean)[0], ac_dim),
                             name="z")  # Cannot pass None as shape param to random_normal, so use `tf.shape(sy_mean)[0]`
    ...
    if nn_baseline:
        ...
        sy_bl_n = tf.placeholder(shape=[None], name="nn_baseline_targets", dtype=tf.float32)
        baseline_loss = tf.nn.l2_loss(baseline_prediction - sy_bl_n)
    ...
    for itr in range(n_iter):
        ...
        if not reward_to_go:
            ...
            for path in paths:
                r_i = path['reward']
                q_is.append([np.sum([np.power(gamma, t_p - t) * r_i[t_p] for t_p in range(t, len(r_i))]) for t in
                             range(len(r_i))])
